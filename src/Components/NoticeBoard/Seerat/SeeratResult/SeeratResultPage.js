...
const SeeratResultPage = () => {
  ...
  const getStudentList = async (searchName) => {
    const {data} = await axios.get(`https://alharamanin-backend-web.onrender.com/seerat/${params.batch}/${searchName}`)
    // http://localhost:5000
...
const b = digitConverter(params.batch);
    ...
    return (
        <div className="my-10">
      <p className="text-center text-2xl">সীরাত কোর্সের {b} তম ব্যাচে স্বাগতম!</p>
