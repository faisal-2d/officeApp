...
if(!student){
    ...
    return (
        <div className='container mx-auto mb-40 px-5'>
            <p className="text-center text-2xl my-10">সীরাত কোর্সের {digitConverter(params.batch)}{thConverter(params.batch)} ব্যাচে স্বাগতম!</p>
            ...
            {student.seerat1payment && <SeeratPaymentCard
                payment={student.seerat1payment}
                level ={1}
                ></SeeratPaymentCard> }
            {student.seerat2payment && <SeeratPaymentCard
...
{/* seerat Certificate  */}
{(student.seeratCertificate || student.seeratCourseCertificates) && <div>

                <p className="text-2xl mt-12 mb-5">অভিনন্দন! আপনি সার্টিফিকেট পেয়েছেন!</p>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-10">

                { student.seeratCourseCertificates?.level1 && <div className={`text-center p-5 rounded bg-green-100 w-fit `}>
                    <img className="rounded" src={aq_certi_1} alt="" />
                    {student.seerat1payment=="due" 
                    ?  <Link className="bg-red-500 block mt-2 text-white p-2 rounded shadow" to='/seerat-due'>বকেয়া পরিশোধ করুন</Link>
                    : <a className="bg-green-500 block mt-2 text-white p-2 rounded shadow" href={student.seeratCourseCertificates?.level1} target="_blank" rel="noopener noreferrer"><FontAwesomeIcon className='pr-5' icon={faFilePdf}/> Level 1 Certificate</a>
                    }
                    
                </div>}
                
                {student.seeratCourseCertificates?.level2 && <div className="text-center p-5 rounded bg-sky-100 w-fit">
                    <img className="rounded" src={aq_certi_2} alt="" />
                    {student.seerat2payment=="due" 
                    ?  <Link className="bg-red-500 block mt-2 text-white p-2 rounded shadow" to='/seerat-due'>বকেয়া পরিশোধ করুন</Link>
                    : <a className="bg-blue-500 block mt-2 text-white p-2 rounded shadow" href={student.seeratCourseCertificates?.level2} target="_blank" rel="noopener noreferrer"><FontAwesomeIcon className='pr-5' icon={faFilePdf}/> Level 2 Certificate</a>
                    }                    
                </div>}                
...
{/* Level 2  */}
            {student.seerat2data && <SeeratLevelComponents
            batch={params.batch}
            sn = {student.sn}
            gender = {student.gender}
            data={student.seerat2data}
            level ={2}
            ></SeeratLevelComponents>}

{/* Level 1  */}
            {student.seerat1data && <SeeratLevelComponents
            batch={params.batch}
            sn = {student.sn}
            gender = {student.gender}
                    ...
                    <div className="my-5  mx-auto">
                        ...
                        <div className="p-10 bg-slate-200 rounded-sm">
                            ...
                            {adminDesk && <div className="my-10">
                                {!student.seerat2data && <button onClick={accessLevelTwo} className="p-2 px-4 m-2 btn-secondary rounded-md"> Access Level 2</button>}
